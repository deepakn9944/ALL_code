...
def initialize_model(model_type, X_train, y_train):
...
feature_subsets = [
    list(range(10)),  # First 10 features (indices)
    list(range(10, 20)),  # Next 10 features (indices)
    list(range(20, 30)),
    list(range(30, X_initial_scaled.shape[1]))  # Remaining features (indices)
]

# Initialize models for the committee
model_types = ['random_forest', 'svm', 'knn', 'bayes']
models = [initialize_model(model_type, X_initial_train[:, subset], y_initial_train)
          for model_type, subset in zip(model_types, feature_subsets)]
