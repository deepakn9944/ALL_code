...
# Function to initialize a RandomForest model
...
oracle_model = RandomForestClassifier(n_estimators=10, random_state=42)
...
for i, batch in enumerate(create_batches(X_train, y_train, batch_size)):
   ...
   # print(f"high entropy samples : {len(top_high_entropy_labels)} low entropy samples: {len(top_low_entropy_labels)} Iteration {i+1}")
